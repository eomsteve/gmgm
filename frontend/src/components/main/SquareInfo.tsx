...
const SSquareInfo: FC<SquareInfoProps> = props => {
  ...
  return (
    <>
      <div
        className={`${textColor} ${gridSize} flex items-center justify-center`}
      >
        {cpi && (
          <div
            className={`card h-full w-full rounded-xl ${
              flip ? `flip bg-[#F75F8E]` : `${bgColor}`
            }`}
          >
            <div className="p-3 text-end" onClick={() => setFlip(!flip)}>
              {flip ? (
            ...
            <div className="back" onClick={() => setFlip(!flip)}>
              <div className="px-2 antialiased">
                각 가정이 <span>생활을 위해 구입하는 상품과 서비스</span>의 가격
                변동을 측정하기 위해 <span>국가</span>에서 작성한 지수
              </div>
            </div>
          </div>
        )}
        {news && (
          <div
            ...
            className={`h-full w-full rounded-xl p-2 ${bgColor}`}
          >
            <div className={`text-center text-lg text-gray-900`}>물가 뉴스</div>
            <div className={`text-base leading-none`}>{news.title}</div>
            <div className={`text-right text-sm`}>{news.pubDate}</div>
          </div>
        )}
        {favoriteIndex &&
          (favoriteIndex.value === -1 ? (
            isLogin ? (
              <div className={`card h-full w-full rounded-xl ${bgColor}`}>
                <div
                  className="p-3 text-end"
                  onClick={() => navigate('/favorite/selection')}
                >
                  <FontAwesomeIcon icon={faPlus} />
                </div>
                <div onClick={() => navigate('/favorite/selection')}>
                  <div className={`front text-center text-xl`}>
                    즐겨찾기에<br></br>상품을 추가하여<br></br>지수를
                    만들어보세요
                  </div>
                </div>
              </div>
              ...
              <div
                className={`card h-full w-full rounded-xl ${
                  flip ? `flip bg-[#FF9887]` : `${bgColor}`
                }`}
                onClick={() => navigate('/LogIn')}
              >
                <div className="front">
                  <div className={`text-center text-2xl`}>
                    즐겨찾기 지수를<br></br>확인하려면<br></br>로그인하세요
                  </div>
                </div>
              </div>
            )
            ...
            >
              ...
              <div className="back" onClick={() => setFlip(!flip)}>
                <div className="px-2 antialiased">
                  <span>즐겨찾기에 추가한 상품</span>의 가격 변동을 측정하기
                  위해 계산한 지수
                </div>
              </div>
            </div>
        ...
        {gmgmIndex && (
          ...
          >
            ...
            <div className="back" onClick={() => setFlip(!flip)}>
              <div className="px-2 antialiased">
                <span>가물가물에서 지정한 89개 품목</span>의 가격 변동을
                측정하기 위해 계산한 지수
              </div>
            </div>
          </div>
