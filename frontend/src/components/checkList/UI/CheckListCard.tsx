  ...
  return (
    <>
      <div className="center container m-2 grid w-[86vw] grid-cols-12 items-center rounded border border-gray-300 py-2 px-2 lg:py-0 ">
        ...
        <label
          className={`${
            isEdit
              ...
              : customProductName
              ? 'col-span-10'
              : basicProductName?.length === 9
              ? ' col-span-4 truncate text-xs '
              : ' col-span-4 '
        ...
        >
          {customProductName || basicProductName}
        </label>
        {isEdit
          ? ''
          : basicProductName && (
              <div className="col-span-2">
                ...
                >
                  {priceGap == undefined
                    ? ''
                    : `${priceGap.toLocaleString()} 원`}
                </span>
              </div>
            )}
        {!isEdit && recentPrice && (
          <span
            className={`col-span-3 mr-2 flex justify-end ${
              Math.round(recentPrice).toLocaleString().length < 5
                ? ''
                : 'text-sm'
            }`}
          >
            {Math.round(recentPrice).toLocaleString()} 원
          </span>
        )}
